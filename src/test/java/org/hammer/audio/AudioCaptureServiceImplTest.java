...
class AudioCaptureServiceImplTest {
  ...
  void getLatestModel_returns_empty_when_no_capture() {
    ...
    AudioCaptureServiceImpl service = new AudioCaptureServiceImpl(16000.0f, 16, 1, true, false, 8);
  ...
  void getLatestModel_returns_same_instance_when_called_multiple_times() {
    // Create service but don't start it (can't reliably test with actual audio capture in unit
    // test)
    AudioCaptureServiceImpl service = new AudioCaptureServiceImpl(16000.0f, 16, 1, true, false, 8);
    ...
    assertSame(
        model1,
        model2,
        "Should return same cached instance when called multiple times without updates");
  }
}
